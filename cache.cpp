...
static auto is_visible(uintptr_t skeletal_mesh) -> bool {
	...
	auto last_submit = read<float>(skeletal_mesh + 0x2E8); //0x2E8
	auto last_render = read<float>(skeletal_mesh + 0x2E0); //0x2E0
	...
	public:
		...
		auto actor_loop() -> void {
			...
			if (globals::g_watermark) {
				...
				DrawString(globals::g_font_size, 10, 10, ImColor(255, 255, 255, 255), false, true, "KRAFTNITE - PRIVATE");
			...
			for (auto& cached : entity_list) {
				...
				if (globals::g_ranked)
				{
					...
					char ranked[64];
					sprintf_s(ranked, verucryptt("%s"), (final));
			...
			fvector ComponentVelocity = read<fvector>(arrays->root_component + offset::velocity); // ComponentVelocity value
			...
			if (globals::g_triggerbot && arrays->player_controller) {
				auto TargetedPawn = read<uintptr_t>(arrays->player_controller + offset::TargetedFortPawn);

				if (TargetedPawn && GetAsyncKeyState(VK_XBUTTON1) & 0x8000 /*add team check if needed*/) {
					INPUT Input = { 0 };
			...
			if (globals::PlayerInfo)
			{
				...
				uintptr_t kills1 = read<uintptr_t>(arrays->player_array + 0x1214);
				uintptr_t Levels = read<uintptr_t>(arrays->player_state + 0x1218);
